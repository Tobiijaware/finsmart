...
<div class="row">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <form method="post" action="/fetchmonth">
                @csrf 
                <div class="row">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="month" name="stop" class="form-control" required>
                    </div>
                    <div class="col-md-3">
                        <label><br></label><br>
                        <input type="submit" class="btn btn-outline-primary" value="SUBMIT FORM" >
                    </div>     
                </div>
        ...
        <div class="card-body">
            <?php if($start){  ?>
                <table class="table color-bordered-table purple-bordered-table">
                    <thead>
                        <tr>
                            <th>SN</td><th>TRANSACTION CATEGORY</th>
                            <?php                                                
                            $yy = date('Y',$start); 
                            $mm1 = date('m',$start);
                            $mm2 = date('m',$stop);
                    ...
                    <tbody>
                      <tr><th></th><th colspan="14">CASH CREDIT</td></tr>
                    <?php $sum = 0 ;  $x=1;
                     $i = 11; $mtotal=0;
                    while($i<=20){   $e = $i++; $title = $fin->walletRemark($e);
                      if(!empty($title)){ $y=$x++;
                          ?>
                       <tr>
                           <td><?php  echo $y ?></td>
                           <td><?php  echo ucwords($title) ?></td>
                           <?php $a=$mm1; $ytotal=0; while($a<=$mm2){$b=$a++; 
                           $bx = $b<10 ? $yy.'0'.$b : $yy.$b; 
                           $bgc = $tr==$bx.$e ? ' style="background-color: yellow"' : ''; ?> 
                    <?php  $ytotal += $fin->monthlyExp2($e,$bx); echo '<td'.$bgc.'><a href="?trid='.rand(1000000000,9999999999).$bx.$e.'">'.number_format($fin->monthlyExp2($e,$bx)).'</a></td>';  }
                    ...
                    <tr>
                        <th></th><th colspan="1">TOTAL CREDIT</th>
                         <?php $a=$mm1; $sum=0; while($a<=$mm2){$b=$a++;
                         $bx = $b<10 ? $yy.'0'.$b : $yy.$b; 
                         $bgc = ($tm==$bx AND $tz==20) ? ' style="background-color: yellow"' : '';
                          $sum += $fin->yearlyExp2($bx);  echo '<th'.$bgc.'><a href="?tmid='.rand(1000000000,9999999999).$bx.$e.'">'.number_format($fin->yearlyExp2($bx)).'</th>'; } ?>
                        <th ><?php  echo number_format($sum) ?></th>
                    </tr>
                      <tr><th></th><th colspan="14">CASH DEBIT</td></tr>
                    <?php $sum = 0 ; 
                     $i = 1; $mtotal=0;  $x=1;
                    while($i<=10){   $e = $i++; $title = $fin->walletRemark($e);
                      if(!empty($title)){ $y=$x++;
                       ...
                       <tr>
                           ...
                           $bx = $b<10 ? $yy.'0'.$b : $yy.$b; 
                           $bgc = $tr==$bx.$e ? ' style="background-color: yellow"' : ''; 
                           $ytotal += $fin->monthlyExp2($e,$bx); echo '<td'.$bgc.'><a href="?trid='.rand(1000000000,9999999999).$bx.$e.'">'.number_format($fin->monthlyExp2($e,$bx)).'</a></td>';  }
                    ...
                    <?php }  } ?>
                    <tr>
                        <th></th><th colspan="1">TOTAL DEBIT</th>
                         <?php $a=$mm1; $sum=0; while($a<=$mm2){$b=$a++;
                         $bx = $b<10 ? $yy.'0'.$b : $yy.$b; 
                         $bgc = ($tm==$bx AND $tz==10) ? ' style="background-color: yellow"' : '';
                          $sum += $fin->yearlyExp1($bx);  echo '<th'.$bgc.'><a href="?tmid='.rand(1000000000,9999999999).$bx.$e.'">'.number_format($fin->yearlyExp1($bx)).'</th>'; } ?>
                        <th ><?php  echo number_format($sum) ?></th>
                    </tr>
                    <tr><th></th><th colspan="14"></td></tr>

                       <tr>
                        <th></th><th colspan="1">PROFIT/LOSS</th>
                         <?php $a=$mm1; $sum=0; while($a<=$mm2){$b=$a++; 
                         $bx = $b<10 ? $yy.'0'.$b : $yy.$b;
